...
if (eventName === "sessionStart") {
  ...
  const updateNotice = body.update?.updateAvailable
    ? ` AI Session Hub ${body.update.latestVersion} is available (installed: ${body.update.currentVersion}). ` +
      `Mention this once at a natural stopping point. Copilot users can run /sesh-update. ` +
  ...
  const projectContext = body.project
    ...
    : "This session is unassigned. Save its checkpoint independently and do not attach it to a project automatically. The user can run /sesh-project to create or choose one. ";
  const context =
