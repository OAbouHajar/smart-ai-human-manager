...
2. Determine the todo list:
   - If the user included todos with the `/sesh-handoff`, `/sesh-next`, or `/wrap-with-next` invocation, use those todos.
...
```text
AI Session Hub <latestVersion> is available (installed: <currentVersion>). Run /sesh-update to prepare it safely.
```
