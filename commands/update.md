---
description: Update Context Workspace safely with one command
---

Schedule the latest stable Context Workspace update. The user must not need to run a second script or handle a temporary path.

1. Find the dashboard URL in the Context Workspace context added when this session started.
...
```json
{ "sessionId": "<current Context Workspace session ID>" }
```
...
10. Do not run another installer command, clone a repository yourself, or show a temporary path. Existing Context Workspace and legacy data plus unrelated provider settings are preserved.
